...
<div class="list-group list-group-flush">
    ...
    if($todoList) {
        foreach ($todoList as $task) {
            ...
            $action  = $task->status ? '/todos/uncompleted' : '/todos/complete';
            echo "<div class='d-flex list-group-item'>";
            echo "    <a href='$action?id={$task->id}' class='w-100 text-decoration-none text-dark'>";
            ...
            echo "    </a>";
            echo "    <a href='/todos/delete?id={$task->id}' type='button' class='p-2'><i class='fa-solid fa-trash text-danger'></i></a>";
            echo "</div>";
        }
     } ?>
</div>
